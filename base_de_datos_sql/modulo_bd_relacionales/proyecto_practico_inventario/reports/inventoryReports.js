class InventoryReports {
  ...
  // Análisis ABC de productos (CORREGIDO)
  async analisisABC() {
    const sql = `
      ...
      SELECT
        ...
        unidades_vendidas,
        valor_vendido,
        ROUND((valor_vendido / GREATEST(total_valor, 1)) * 100, 2) AS porcentaje_valor,
        CASE
          WHEN SUM(valor_vendido) OVER (ORDER BY valor_vendido DESC) / GREATEST(total_valor, 1) <= 0.80 THEN 'A'
          WHEN SUM(valor_vendido) OVER (ORDER BY valor_vendido DESC) / GREATEST(total_valor, 1) <= 0.95 THEN 'B'
          ELSE 'C'
