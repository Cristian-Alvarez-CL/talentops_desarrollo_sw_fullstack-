...
INSERT INTO productos (codigo, nombre, descripcion, precio_compra, precio_venta, stock_actual, stock_minimo, stock_maximo, categoria_id, proveedor_id, activo)
VALUES ('TEST-CRITICO', 'Monitor UltraWide 34"', 'Producto de prueba para stock bajo', 300.00, 599.99, 2, 10, 50, 1, 1, 1);

SET @id_producto_critico = LAST_INSERT_ID();

INSERT INTO ordenes_compra (numero_orden, proveedor_id, fecha_orden, fecha_entrega_esperada, estado, total) VALUES 
('OC-TEST-SIMULACION', 1, CURDATE(), DATE_ADD(CURDATE(), INTERVAL 7 DAY), 'pendiente', 15000.00);

SET @id_orden = LAST_INSERT_ID();

INSERT INTO detalle_ordenes_compra 
(orden_compra_id, producto_id, cantidad_solicitada, cantidad_recibida, precio_unitario, subtotal) VALUES 
(@id_orden, @id_producto_critico, 50, 0, 300.00, 15000.00);